...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func sandboxArchivePath() -> String {
        ...
        return dir.appendingPathComponent("luckynumbers.plist")
    }
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        if FileManager.default.fileExists(atPath: archiveName) {
            ...
            let saveState = NSArray(contentsOfFile: archiveName) as! [[Int]]
            board!.state = saveState
        }
        else {
            self.board!.scramble(numTimes: numShuffles)
        }
    ...
    func applicationDidEnterBackground(_ application: UIApplication) {
        ...
        let archiveName = sandboxArchivePath()
        let saveState : NSArray = board!.state as NSArray
